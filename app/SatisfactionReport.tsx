...
export default function SatisfactionReport({ dashboard, details, detailsError = "" }: { dashboard: CategoryDashboard; details: SatisfactionComment[]; detailsError?: string }) {
  ...
  const [commentTone, setCommentTone] = useState<"all" | "positive" | "negative" | "proposals">("all");
  ...
  const monthOptions = [{ value: "all", label: "Todos" }, ...[...new Set(rows.filter((row) => year === "all" || row.mes.startsWith(year)).map((row) => row.mes.slice(5, 7)))].sort().map((value) => ({ value, label: monthNames[Number(value) - 1] }))];
  const matchesPeriod = (row: SatisfactionMetricRow) => (year === "all" || row.mes.startsWith(year)) && (month === "all" || row.mes.slice(5, 7) === month);
  const regionScopeRows = rows.filter((row) => matchesPeriod(row) && (program === "" || program === "all" || row.programa === program));
  const availableRegions = unique(regionScopeRows, "region").filter((value) => value.trim() && textKey(value) !== "sin region");
  const regionAvailable = Boolean(program) && availableRegions.length > 0;
  const regionLabel = program && !regionAvailable ? "Región no disponible" : "Región";
  ...
  const periodReady = year !== "all" && monthChosen;
  ...
  const selectedSentiment = commentTone === "proposals" ? "negative" : commentTone;
  const recentComments = matchingDetails.filter((item) => item.record_type === "comment" && (selectedSentiment === "all" || item.sentiment === selectedSentiment) && isSubstantiveComment(item.comentario));
  const matchingThemes = matchingDetails.filter((item) => item.record_type === "theme" && (selectedSentiment === "all" || item.sentiment === selectedSentiment) && isSubstantiveComment(item.example));
  ...
  return <>
    <div className="floating-toolbar-frame"><div className={instructorMode ? "survey-toolbar people custom-filters" : "survey-toolbar custom-filters"} aria-label="Filtros de satisfacción">
      {instructorMode ? <>
        <div className="instructor-period-filters"><PopupFilter label="Año" value={year === "all" ? undefined : year} options={years.map((value) => ({ value, label: value }))} open={openFilterMenu === "instructor-year"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "instructor-year" : current === "instructor-year" ? null : current)} onChange={(value) => { setYear(value); setMonth("all"); setMonthChosen(false); setExpandedInstructorCourse(null); }} /><PopupFilter label="Mes" className="month-filter" value={monthChosen ? month : undefined} options={monthOptions} open={openFilterMenu === "instructor-month"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "instructor-month" : current === "instructor-month" ? null : current)} onChange={(value) => { setMonth(value); setMonthChosen(true); setExpandedInstructorCourse(null); }} /></div>
        ...
        <PopupFilter label="Año" value={year === "all" ? undefined : year} options={years.map((value) => ({ value, label: value }))} open={openFilterMenu === "year"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "year" : current === "year" ? null : current)} onChange={(value) => { setYear(value); setMonth("all"); setMonthChosen(false); setProgram(""); setRegion("all"); setRegionChosen(false); setExpandedProgram(null); }} />
        <PopupFilter label="Mes" className="month-filter" value={monthChosen ? month : undefined} options={monthOptions} open={openFilterMenu === "month"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "month" : current === "month" ? null : current)} onChange={(value) => { setMonth(value); setMonthChosen(true); setRegion("all"); setRegionChosen(false); }} />
        <PopupFilter label="Programa" className="program-filter" value={program || undefined} options={[{ value: "all", label: "Todos" }, ...unique(rows, "programa").map((value) => ({ value, label: value }))]} open={openFilterMenu === "program"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "program" : current === "program" ? null : current)} onChange={(value) => { setProgram(value); setRegion("all"); setRegionChosen(false); setExpandedProgram(value === "all" ? null : value); }} />
        <PopupFilter label={regionLabel} className="region-filter" disabled={!periodReady || !regionAvailable} value={regionAvailable && regionChosen ? region : undefined} options={[{ value: "all", label: "Todas" }, ...availableRegions.map((value) => ({ value, label: value }))]} open={regionAvailable && openFilterMenu === "region"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "region" : current === "region" ? null : current)} onChange={(value) => { setRegionChosen(true); setRegion(value); setExpandedProgram(program === "all" ? null : program); }} />
