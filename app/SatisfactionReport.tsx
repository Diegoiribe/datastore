...
export default function SatisfactionReport({ dashboard }: { dashboard: CategoryDashboard }) {
  ...
  const selectionReady = periodReady && (instructorMode ? selectedInstructorKeys.length > 0 : program !== "all");
  ...
  const closeInstructorMode = () => { setInstructorMode(false); setSelectedInstructorKeys([]); setInstructorSearch(""); setInstructorSearchOpen(false); setCommentTone("all"); };
  ...
  return <>
    <div className="floating-toolbar-frame"><div className={instructorMode ? "survey-toolbar people custom-filters" : "survey-toolbar custom-filters"} aria-label="Filtros de satisfacción">
      {instructorMode ? <>
        ...
        <PopupFilter label="Año" value={year === "all" ? undefined : year} options={years.map((value) => ({ value, label: value }))} open={openFilterMenu === "year"} onOpenChange={(open) => setOpenFilterMenu((current) => open ? "year" : current === "year" ? null : current)} onChange={(value) => { setYear(value); setMonth("all"); setProgram("all"); setExpandedProgram(null); }} />
    ...
    <section className={program !== "all" ? "survey-grid program-focused" : "survey-grid"}>
